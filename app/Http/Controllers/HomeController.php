...
class HomeController extends Controller
{
    ...
    public function faculties(){
        $faculties = Faculty::orderBy('order_index')->get();
        return view('client.faculties.index', [
            'faculties' => $faculties,
        ]);
    }
    public function faculty(Request $request, $id){
        $pageId = $request->page;
        if($pageId){
            $page = FacultyPage::findOrFail($pageId);
        }
        $childId = $request->child;
        if($childId){
            $child = FacultyPage::findOrFail($childId);
        }
        $faculty = Faculty::findOrFail($id);
        $pages = FacultyPage::where('faculty_id', $id)
                        ->where('parent_id', null)
                        ->orderBy('order_index')
                        ->get();
        return view('client.faculties.show', [
            'faculty' => $faculty,
            'pages' => $pages,
            'page' => $page ?? null,
            'child' => $child ?? null,
    ...
    public function facultyPage(Request $request, $id){
        $page = FacultyPage::findOrFail($id);
        $childId = $request->child;
        if($childId){
            $child = FacultyPage::findOrFail($childId);
        }
        $faculty = Faculty::findOrFail($page->faculty_id);
        $pages = FacultyPage::where('parent_id', $id)
                        ->orderBy('order_index')
                        ->get();
        return view('client.faculties.page', [
            'faculty' => $faculty,
            'page' => $page,
            'child' => $child ?? null,
            'pages' => $pages,
    ...
    public function menus(){
        $pages = MenuPage::orderBy('order_index')->get();
    ...
    public function menu(Request $request, $id) {
        ...
        if($pageId){
            $page = MenuPage::findOrFail($pageId);
        }

        $childId = $request->childId;
        if($childId){
            $child = MenuPage::findOrFail($childId);
        }
        $menu = Menu::findOrFail($id);
        $pages = MenuPage::where('menu_id', $id)
                        ->where('parent_id', null)
                        ->orderBy('order_index')
                        ->get();
        // return [
    ...
    public function index() {
        ...
        $scholarships = Journal::orderBy('id', 'desc')->limit(3)->get();
