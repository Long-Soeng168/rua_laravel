    ...
    <form class="w-full" action="{{ route('admin.items.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid lg:grid-cols-2 lg:gap-6">
            ...
            <div class="relative w-full mb-5 group">
                <x-input-label for="faculty_id" :value="__('Faculties')" />
                <div class="flex flex-1 gap-1 mt-1">
                    <div class="flex justify-start flex-1">
                        <x-select-option wire:model.live='faculty_id' id="faculty_id"
                            name="faculty_id" class="faculty_select">
                            <option wire:key='category' value="0">Select Faculty...</option>
                            @forelse ($categories as $category)
                                <option wire:key='{{ $category->id }}' value="{{ $category->id }}">
                                    {{ $category->name }} {{ ' / ' . $category->name_kh }}
                                </option>
                            @empty
                                <option wire:key='nocateogry' value="0"> --No Faculty--</option>
                            @endforelse
                        </x-select-option>
                    </div>
                    ...
                    <div class="flex justify-start flex-1">
                        <x-select-option wire:model.live='parent_id'
                            ...
                            <option wire:key='sub-category' value="">
                                {{ $faculty_id ? 'Select Parent Page...' : 'Select Faculty First' }}
                            </option>
